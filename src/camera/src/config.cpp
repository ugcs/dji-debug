...
#define APP_VERSION(arg) APP_VERSION_(arg)
	...
	CLI::App app{app_description, "DJI camera"};
	...
	app.add_option("-p, --camera-position", camera_position, "Camera position magic number: 0-4")
				->default_val(DJI_MOUNT_POSITION_PAYLOAD_PORT_NO1);
...
E_DjiMountPosition
ParamsCLI::getCameraPosition() const
{
